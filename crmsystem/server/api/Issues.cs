...
public class Issues
{
    ...
    public Issues(WebApplication app, NpgsqlDataSource db, string url)
    {
        ...
        app.MapGet(url + "/{issueId}", GetIssue)
            .RoleAuthorization(Role.GUEST, Role.USER, Role.ADMIN);
        app.MapPut(url + "/{issueId}/state", UpdateIssueState)
            .RoleAuthorization(Role.USER, Role.ADMIN);
        app.MapGet(url + "/{issueId}/messages", GetMessages)
            .RoleAuthorization(Role.GUEST, Role.USER, Role.ADMIN);
        app.MapPost(url + "/{issueId}/messages", CreateMessage)
            .RoleAuthorization(Role.GUEST, Role.USER, Role.ADMIN);
    ...
    private async Task<IResult> GetIssueByCompany(HttpContext context)
    {
        ...
        await using var cmd = Db.CreateCommand(
            "SELECT * FROM companies_issues WHERE company_name = @company"
        );
    ...
    private async Task<IResult> GetIssue(Guid issueId, HttpContext context)
    {
        ...
        await using var cmd = Db.CreateCommand(
            "SELECT * FROM companies_issues WHERE id = @issue_id AND company_name = @company_name"
        );
    ...
    private async Task<IResult> UpdateIssueState(
        Guid issueId,
        HttpContext context,
        UpdateIssueStateRequest updateIssueStateRequest
    )
    {
        var user = JsonSerializer.Deserialize<User>(context.Session.GetString("User"));

        await using var cmd = Db.CreateCommand(
            "UPDATE issues SET state = @state::issue_state WHERE id = @issue_id AND company_id = @company_id"
        );
        cmd.Parameters.AddWithValue(
            "@state",
            Enum.Parse<IssueState>(updateIssueStateRequest.NewState).ToString()
        );
        cmd.Parameters.AddWithValue("@issue_id", issueId);
        ...
        try
        {
            ...
            else
            {
                return Results.Conflict(
                    new { message = "Query executed but something went wrong." }
                );
            }
        }
        catch (Exception ex)
    ...
    private async Task<IResult> GetMessages(Guid issueId, HttpContext context)
    {
        ...
        await using var cmd =
            user.Role == Role.GUEST
                ? Db.CreateCommand(
                    "SELECT * FROM issues WHERE id = @issue_id AND customer_email = @customer_email"
                )
                : Db.CreateCommand(
                    "SELECT * FROM issues WHERE id = @issue_id AND company_id = @company_id"
                );
    ...
    private async Task<IResult> CreateIssue(
        string companyName,
    ...
    )
    {
        ...
        try
        {
            ...
            await using var cmd2 = Db.CreateCommand(
                "INSERT INTO issues (company_id, customer_email, title, subject, state, created) VALUES (@company_id, @customer_email, @title, @subject, 'NEW', current_timestamp) RETURNING id;"
            );
            ...
            if (issuesId is not null)
            {
                await using var cmd3 = Db.CreateCommand(
                    "INSERT INTO messages (issue_id, message, sender, username, time) VALUES (@issue_id, @message, 'CUSTOMER', @username, current_timestamp)"
                );
                ...
                {
                    Console.WriteLine(ex.Message);
                    await using var cmd4 = Db.CreateCommand(
                    ...
                    return Results.Conflict(
                        new
                        {
                            message = "Issue was created, but something went wrong during the process, so the issue has been deleted.",
                        }
                    );
                }
            }
    ...
    private async Task<IResult> CreateMessage(
        Guid issueId,
        HttpContext context,
    ...
    )
    {
        ...
        {
            return Results.Conflict(
                new { message = "You dont have access to post a message to this issue." }
            );
        }

        await using var cmd2 = Db.CreateCommand(
            "INSERT INTO messages (issue_id, message, sender, username, time) VALUES (@issue_id, @message, @sender::sender, @username, current_timestamp)"
        );
        cmd2.Parameters.AddWithValue("@issue_id", issueId);
    ...
    private string IssueCreatedMessage(
        string companyName,
        string message,
        string title,
        string chatId
