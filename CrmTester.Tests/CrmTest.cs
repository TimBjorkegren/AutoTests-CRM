...
public class DemoTest : PageTest
{
    ...
    public async Task Setup()
    {
        ...
        _browser = await _playwright.Chromium.LaunchAsync(
            new BrowserTypeLaunchOptions { Headless = false, SlowMo = 1000 }
        );
    ...
    public async Task RegisterCompanyAccount()
    {
        ...
        else
        {
            ...
            bool logoutButtonIsVisible = await logoutButton.IsVisibleAsync();
            Assert.IsTrue(logoutButtonIsVisible, "logout button was not visible");
        }
    }
    /*
        [TestMethod]
